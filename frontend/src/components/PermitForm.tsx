...
import {
  ...
  COMMUNICATION_METHODS, MOCK_HIRAS, MOCK_WORKERS, PDF_CHECKLISTS
...
export const PermitForm: React.FC<PermitFormProps> = ({
 ...
 const [endDate, setEndDate] = useState('2026-07-16T16:00');
  ...
  // A permit authorizes a bounded window of work, not an open-ended one — real PTW practice
  // caps initial validity at a single shift and requires a logged extension for anything longer.
  ...
  const dateRangeError = React.useMemo(() => {
    ...
    if (hours > MAX_INITIAL_PERMIT_HOURS) {
      return language === 'ar'
        ? `مدة التصريح الأولية لا يجب أن تتجاوز ${MAX_INITIAL_PERMIT_HOURS} ساعة. للعمل الأطول، استخدم "طلب تمديد" بعد الإصدار بدلاً من صلاحية مفتوحة.`
        : `Initial permit validity should not exceed ${MAX_INITIAL_PERMIT_HOURS} hours. For longer work, use the logged "Request Extension" action after issuing instead of an open-ended window.`;
    }
    return '';
 ...
 const [neboshChecklists, setNeboshChecklists] = useState<{
 ...
 case 4: return competencyWorkers.length > 0;
 default: return true;
 }
 };
 ...
 electricalApprovedAt: undefined,
 lotoRequired: !!neboshChecklists.ppe,
 lotoLockNumber: '',
 lotoKeyNumber: '',
 lotoDetails: [],
 ...
 <input type="datetime-local" value={endDate} onChange={e => setEndDate(e.target.value)} className={inputClass} />
  ...
  {/* ──── STEP 2: NEBOSH Checklists ──── */}
  {currentStep === 2 && (
  <div className="space-y-5">
  {sectionTitle(<CheckCircle className="w-4 h-4 text-orange-500" />, 'الخطوة ٢: قوائم التحقق (الإجراءات المطلوب مراجعتها)', 'Step 2: Required Safety Checklists')}
  ...
  {[
    { id: 'ppe', ar: '(A) مهمات الوقاية', en: '(A) PPE' },
    { id: 'general', ar: '(B) عام', en: '(B) General' },
    { id: 'hotWork', ar: '(C) أعمال ساخنة', en: '(C) Hot Work' },
    { id: 'hotMaterial', ar: '(D) أعمال مواد ساخنة', en: '(D) Hot Materials' },
    { id: 'lifting', ar: '(E) أعمال رفع', en: '(E) Lifting' },
    { id: 'confinedSpace', ar: '(F) دخول أماكن مغلقة', en: '(F) Confined Space' },
    { id: 'workAtHeight', ar: '(G) عمل على ارتفاع', en: '(G) Work at Height' }
  ].map(chk => {
    const isChecked = !!neboshChecklists[chk.id as keyof typeof neboshChecklists];
    return (
    <label key={chk.id} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
      ...
      checked={isChecked}
      onChange={(e) => {
        setNeboshChecklists(prev => {
          const next = { ...prev };
          if (e.target.checked) {
            next[chk.id as keyof typeof neboshChecklists] = {};
          } else {
            delete next[chk.id as keyof typeof neboshChecklists];
          }
          return next;
        });
      }}
  ...
  {/* Checklist Details */}
  {Object.entries(neboshChecklists).filter(([_, itemsMap]) => !!itemsMap).length > 0 && (
    <div className="mt-6 flex flex-col gap-6">
      {Object.entries(neboshChecklists).map(([listId, itemsMap]) => {
...
{/* Gas Readings (conditional) */}
 ...
 <label className="block text-xs font-bold text-red-700 dark:text-red-400 mb-3 flex items-center gap-1.5 justify-start ">
 ...
 { labelAr: 'التوقيعات', labelEn: 'Signatures', value: `${[signatures.issuerName, signatures.receiverName].filter(Boolean).length}/2` },
 ].map((item, idx) => (
